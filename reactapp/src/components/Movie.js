...
import '../App.css';
import { Col, ButtonGroup, Button, Card, CardImg, CardBody, CardTitle, Badge, CardText} from 'reactstrap';
 ...
 var setMyRating = (rating) => {
  ...
  return (
    ...
    <Col xs="12" lg="6" xl="4">
      ...
      <CardImg top src={props.movieImg} alt={props.movieImg}/> 
        <CardBody>
          <p>Like <FontAwesomeIcon style={colorLike} icon={faHeart} onClick={() => changeLiked(props.movieName, props.movieImg)} /></p>
          <p>Nombre de vues <FontAwesomeIcon style={colorWatch} icon={faVideo} onClick={() => addWatch()} /> <Badge variant="secondary">{countWatchMovie}</Badge></p>
          <p>Mon avis 
          {tabRating}
            <ButtonGroup size="sm">
